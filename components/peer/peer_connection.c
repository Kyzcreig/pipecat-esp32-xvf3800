...
void peer_connection_set_remote_description(PeerConnection* pc, const char* sdp, SdpType type) {
  ...
  while ((line = strstr(start, "\r\n"))) {
    ...
    if (line_len >= sizeof(buf)) {
      LOGW("Ignoring oversized SDP line (%zu bytes)", line_len);
      start = line + 2;
      continue;
    }
...
static const char* peer_connection_create_sdp(PeerConnection* pc, SdpType sdp_type) {
  ...
  switch (pc->config.audio_codec) {
    ...
    case CODEC_PCMU:
      ...
      break;
    case CODEC_OPUS:
      // VENDORED PATCH: offer RED (RFC 2198) alongside Opus so the downlink
      // can carry two preceding frames. This is the upstream Opus SDP with
      // dynamic payload type 63 added; the microphone uplink remains PT 111.
      // The offer is inert when the server selects plain Opus.
      ...
      // Three entries negotiate two redundant Opus generations plus primary.
      sdp_append(pc->sdp, "a=fmtp:63 111/111/111");
      // Recovery and playback framing are fixed at one 20 ms Opus frame/packet.
      sdp_append(pc->sdp, "a=ptime:20");
