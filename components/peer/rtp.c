...
volatile uint32_t g_rtp_stream_resets = 0;

// RED / RFC 2198 counters. packets_received is the denominator for transport
// recovery burden: (plc + fec_attempts + red_recovered) / packets_received.
volatile uint32_t g_rtp_packets_received = 0;
volatile uint32_t g_red_recovered = 0;
volatile uint32_t g_red_dup_drops = 0;
volatile uint32_t g_red_parse_failures = 0;
volatile uint32_t g_red_profile_mismatches = 0;
...
static int rtp_decode_generic(RtpDecoder* rtp_decoder, uint8_t* buf, size_t size) {
  ...
  // RED is parsed before sequence handling so redundant blocks can replace
  // missing frames in timestamp order. Malformed PT 63 framing is one lost
  // frame; RED headers must never be reinterpreted as a bare Opus packet.
  ...
  if (rtp_packet->header.type == RED_PAYLOAD_TYPE) {
    if (red_unwrap(payload, payload_size, PT_OPUS, &red) == 0) {
      ...
      g_red_parse_failures++;
    }
    // red_unwrap guarantees a zeroed output on failure, which turns this
    // packet into the same (NULL, 0) loss signal used by uncovered gaps.
    payload = (uint8_t*)red.primary;
    payload_size = red.primary_size;
  }

  // --- VENDORED PATCH (2026-07-09, pipecat-esp32-xvf3800) ---
  ...
  // Fix: track seq in the decoder; on a gap, signal the consumer BEFORE the new
  // payload with (NULL, 0) callbacks so it can run opus PLC
  // (opus_decode(dec, NULL, ...)) once per missing frame unless RED carries a
  // lossless copy. Late/duplicate packets are dropped because playback has
  // moved on. rtp_decoder_init starts a new epoch; an SSRC change does too.
  uint16_t seq = ntohs(rtp_packet->header.seq_number);
  uint32_t ts = ntohl(rtp_packet->header.timestamp);
  uint32_t ssrc = ntohl(rtp_packet->header.ssrc);
  RtpSequenceEvent event =
      rtp_sequence_update(&rtp_decoder->sequence, seq, ts, ssrc);
  if (event.kind == RTP_SEQUENCE_STREAM_RESET) {
    g_rtp_stream_resets++;
  }
  if (event.kind == RTP_SEQUENCE_LATE) {
    // Late or duplicate packet — playback has moved on; drop it. Never feed
    // redundancy after a newer packet has advanced the Opus decoder state.
    g_rtp_late_drops++;
    if (red_ok) {
      g_red_dup_drops++;
    }
  ...
  int16_t delta = event.gap;
  if (event.kind == RTP_SEQUENCE_GAP && delta <= RED_MAX_GAP &&
      rtp_decoder->on_packet != NULL) {
    // Gap: cap recovery at 16 packets. Feed matching RED blocks oldest
    // first; uncovered frames retain the existing PLC/FEC signal.
    g_rtp_gap_events++;
    int8_t actions[RED_MAX_GAP];
    int planned = 0;
    if (red_ok) {
      // This target supports fixed 20 ms Opus. Require an exact timestamp
      // advance before selecting RED blocks; mixed ptime or a discontinuity
      // fails safely to FEC/PLC and gets its own diagnostic counter.
      uint32_t ts_step = 0;
      if (red_validate_timestamp_advance(
              event.previous_timestamp, ts, delta,
              RED_OPUS_TS_STEP_20MS, &ts_step) == 0) {
        planned = red_recover_plan(&red, delta, ts_step, actions);
      } else {
        g_red_profile_mismatches++;
      }
    }
    for (int16_t i = 0; i < delta; i++) {
      if (i < planned && actions[i] >= 0) {
        RedBlock* block = &red.blocks[actions[i]];
        // This callback is a real chronological Opus frame. If earlier gaps
        // were uncovered, media.cpp uses this frame as their next-packet FEC
        // opportunity before decoding it normally.
        rtp_decoder->on_packet((uint8_t*)block->data, block->length,
...
void rtp_decoder_init(RtpDecoder* rtp_decoder, MediaCodec codec, RtpOnPacket on_packet, void* user_data) {
  ...
  rtp_sequence_init(&rtp_decoder->sequence);
