  ...
  switch (hash(j_type->valuestring)) {
    ...
    case hash("server-message"): {
      ...
      if (hash(j_t->valuestring) == hash("led")) {
        ...
        // Application-level liveness: echo the nonce in a canonical RTVI
        // client-message envelope so server app-message handlers receive it.
        ...
        if (pong_data == NULL ||
            ...
            cJSON_AddNumberToObject(pong_data, "nonce",
                                    j_nonce->valuedouble) == NULL) {
          destroy_rtvi_message(pong);
          break;
        }
        char *pong_str = rtvi_message_to_string(pong);
        if (pong_str != NULL) {
          peer_connection_datachannel_send(peer_connection, pong_str,
                                           strlen(pong_str));
          cJSON_free(pong_str);
        }
        destroy_rtvi_message(pong);
